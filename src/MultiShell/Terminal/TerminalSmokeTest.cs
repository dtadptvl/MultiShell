...
internal static class TerminalSmokeTest
{
    ...
    private static async Task<int> VerifyNativeBracketedPasteAsync()
    {
        var tempDir = Path.Combine(
            Path.GetTempPath(),
            "MultiShell-Smoke-" + Guid.NewGuid().ToString("N"));
        ...
        File.WriteAllText(
            ...
            using System;
            using System.Runtime.InteropServices;
            public static class MultiShellRawConsole {
            ...
            $stdin = [MultiShellRawConsole]::GetStdHandle(-10)
            [uint32]$mode = 0
            if (-not [MultiShellRawConsole]::GetConsoleMode($stdin, [ref]$mode)) {
            ...
            if (-not [MultiShellRawConsole]::SetConsoleMode($stdin, $rawMode)) {
        ...
        File.WriteAllText(
            ...
            using System;
            using System.Runtime.InteropServices;
            public static class MultiShellWideConsole {
                [DllImport("kernel32.dll", SetLastError = true)]
                public static extern IntPtr GetStdHandle(int nStdHandle);
                [DllImport("kernel32.dll", SetLastError = true)]
                [return: MarshalAs(UnmanagedType.Bool)]
            ...
            $stdin = [MultiShellWideConsole]::GetStdHandle(-10)
            [uint32]$mode = 0
            if (-not [MultiShellWideConsole]::GetConsoleMode($stdin, [ref]$mode)) {
                throw "GetConsoleMode failed"
            }
            ...
            if (-not [MultiShellWideConsole]::SetConsoleMode($stdin, $wideMode)) {
                throw "SetConsoleMode failed"
            }
            ...
            while ($builder.Length -lt $expected.Length) {
                [uint32]$read = 0
                if (-not [MultiShellWideConsole]::ReadConsoleW($stdin, $buffer, [uint32]$buffer.Length, [ref]$read, [IntPtr]::Zero)) {
