...
MultiShell embeds the native Windows Terminal `HwndTerminal` renderer/input engine. It does not fork or build the Windows Terminal application.
...
The WPF host is also pinned to the same upstream revision. Keyboard, TSF/IME, selection, mouse behavior, renderer state and VT parsing remain owned by Windows Terminal components. MultiShell intentionally keeps DECSET 9001 (Win32 input mode) disabled so keyboard input uses the normal VT path expected by Kilo.
