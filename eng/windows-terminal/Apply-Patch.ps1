...
Write-Host "Applied MultiShell native terminal patch to pinned Windows Terminal source."


# ---------------------------------------------------------------------------
# MultiShell bridge to Windows Terminal's real ConptyConnection
# ---------------------------------------------------------------------------
...
$connectionHppNew = @'
        uint64_t RootProcessHandle() noexcept;
        void MultiShellSetBridgeMode(bool enabled) noexcept;
        void MultiShellSetJob(HANDLE job) noexcept;
        DWORD MultiShellLastExitCode() const noexcept;
'@
...
'@
$launchReplacement = @'
    void ConptyConnection::MultiShellSetBridgeMode(bool enabled) noexcept
    {
        _multiKiloBridgeMode = enabled;
    }

    void ConptyConnection::MultiShellSetJob(HANDLE job) noexcept
    {
        _multiKiloJob = job;
    }

    DWORD ConptyConnection::MultiShellLastExitCode() const noexcept
    {
...
// MultiShell uses this tiny C ABI to consume the exact Windows Terminal
// ConptyConnection implementation without requiring packaged WinRT activation.
namespace
{
    using MultiShellOutputCallback = void(__stdcall*)(void*, LPCWSTR, uint32_t);
    using MultiShellExitCallback = void(__stdcall*)(void*, DWORD);

    struct MultiShellConptySession
    {
        ...
        std::atomic<MultiShellOutputCallback> outputCallback{ nullptr };
        std::atomic<MultiShellExitCallback> exitCallback{ nullptr };
...
extern "C" __declspec(dllexport) HRESULT __stdcall MultiShellConptyCreate(
    LPCWSTR commandLine,
    LPCWSTR workingDirectory,
    uint32_t columns,
    uint32_t rows,
    void* context,
    MultiShellOutputCallback outputCallback,
    MultiShellExitCallback exitCallback,
    void** session)
try
{
    ...
    *session = nullptr;
    ...
    auto holder = std::make_unique<MultiShellConptySession>();
    ...
    holder->connection->MultiShellSetBridgeMode(true);
    holder->connection->MultiShellSetJob(holder->job.get());
    ...
    holder->stateToken = holder->connection->StateChanged([raw](const auto&, const auto&) {
            ...
            state == winrt::Microsoft::Terminal::TerminalConnection::ConnectionState::Failed)
        {
            ...
            {
                callback(callbackContext, raw->connection->MultiShellLastExitCode());
            }
        }
    });
...
extern "C" __declspec(dllexport) void __stdcall MultiShellConptyWrite(
    void* session,
    LPCWSTR data,
    uint32_t length)
try
{
    const auto holder = static_cast<MultiShellConptySession*>(session);
...
extern "C" __declspec(dllexport) void __stdcall MultiShellConptyResize(
    void* session,
    uint32_t columns,
    uint32_t rows)
try
...
extern "C" __declspec(dllexport) void __stdcall MultiShellConptyTerminate(void* session)
try
{
    const auto holder = static_cast<MultiShellConptySession*>(session);
    if (holder)
...
extern "C" __declspec(dllexport) BOOL __stdcall MultiShellConptyIsRunning(void* session)
{
    const auto holder = static_cast<MultiShellConptySession*>(session);
    return holder && holder->running ? TRUE : FALSE;
}

extern "C" __declspec(dllexport) void __stdcall MultiShellConptyDestroy(void* session)
try
{
    std::unique_ptr<MultiShellConptySession> holder{ static_cast<MultiShellConptySession*>(session) };
    if (!holder)
    {
        return;
    }
...
Write-Host "Patched upstream TerminalConnection for MultiShell."


# ---------------------------------------------------------------------------
# MultiShell native session backend
# ---------------------------------------------------------------------------
...
$structNew = "class MultiShellTerminalConnectionSession;" + [Environment]::NewLine + [Environment]::NewLine + $structOld
...
$hppStateNew = @'
    std::function<void(wchar_t*)> _pfnWriteCallback;
    std::unique_ptr<MultiShellTerminalConnectionSession> _nativeSession;
...
$nativeSessionCode = @'
class MultiShellTerminalConnectionSession
{
public:
    ...
    explicit MultiShellTerminalConnectionSession(HwndTerminal* owner) noexcept :
        _owner(owner)
    {
    }

    ~MultiShellTerminalConnectionSession()
    {
        Close();
    }
    ...
    HRESULT Start(std::wstring_view commandLine, std::wstring_view workingDirectory, uint32_t columns, uint32_t rows) noexcept
    {
        ...
        _create = reinterpret_cast<CreateFn>(GetProcAddress(_module, "MultiShellConptyCreate"));
        _write = reinterpret_cast<WriteFn>(GetProcAddress(_module, "MultiShellConptyWrite"));
        _resize = reinterpret_cast<ResizeFn>(GetProcAddress(_module, "MultiShellConptyResize"));
        _terminate = reinterpret_cast<TerminateFn>(GetProcAddress(_module, "MultiShellConptyTerminate"));
        _isRunning = reinterpret_cast<IsRunningFn>(GetProcAddress(_module, "MultiShellConptyIsRunning"));
        _destroy = reinterpret_cast<DestroyFn>(GetProcAddress(_module, "MultiShellConptyDestroy"));
...
private:
    ...
    {
        const auto self = static_cast<MultiShellTerminalConnectionSession*>(context);
...
try
{
    ...
    publicTerminal->_nativeSession = std::make_unique<MultiShellTerminalConnectionSession>(publicTerminal);
...
Write-Host "Applied MultiShell native ConPTY session backend."
