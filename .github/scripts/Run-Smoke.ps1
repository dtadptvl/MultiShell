...
$exe = ".\src\MultiShell\bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\MultiShell.exe"
if (-not (Test-Path $exe)) {
    throw "Built MultiShell.exe not found at $exe"
}

$log = Join-Path (Split-Path $exe) "smoke-error.txt"
Remove-Item $log -ErrorAction SilentlyContinue

$dumpRoot = Join-Path $env:RUNNER_TEMP "multishell-dumps"
New-Item -ItemType Directory -Force -Path $dumpRoot | Out-Null
$dumpKey = "HKCU:\Software\Microsoft\Windows\Windows Error Reporting\LocalDumps\MultiShell.exe"
...
if ($process.ExitCode -ne 0) {
    ...
    $terminalCrash = $recentEvents |
        Where-Object { $_.Message -match "Faulting application name: MultiShell\.exe" -and $_.Message -match "Faulting module name: TerminalConnection\.dll" } |
    ...
    $dumps = @(Get-ChildItem $dumpRoot -Filter "MultiShell*.dmp" -File -ErrorAction SilentlyContinue)
